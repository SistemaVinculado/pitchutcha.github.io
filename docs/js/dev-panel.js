// docs/js/dev-panel.js (Versão 1.9.0 - Unificada e Definitiva)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Carrega script do Axe para o painel principal (usado na aba Auditoria)
    ...
    const analyzePageInIframe = (iframe, url) => {
        return new Promise(resolve => {
            ...
            const onIframeLoad = async () => {
                ...
                try {
                    // A CORREÇÃO CRÍTICA: Injeta o script do Axe no iframe e espera carregar
                    if (typeof axe !== 'undefined') {
                        await new Promise((resolveAxe) => {
                            const axeIframeScript = doc.createElement('script');
                            axeIframeScript.src = `${baseUrl}js/vendor/axe.min.js`;
                            axeIframeScript.onload = resolveAxe;
                            doc.head.appendChild(axeIframeScript);
                        });
                        const axeResults = await win.axe.run(doc.documentElement, { resultTypes: ['violations', 'incomplete'] });
                    ...
                    if(perf && perf.domContentLoadedEventEnd > 2000) issues.performance.push({ text: `DOM Content Loaded Lento: ${perf.domContentLoadedEventEnd.toFixed(0)}ms`});
                     ...
                     issues.jsErrors.push({ message: `Erro ao analisar a página com Axe: ${e.message}` });
                } finally {
                    formatAndResolve();
                }
            };
            ...
            iframe.contentWindow.onerror = (message) => { issues.jsErrors.push({ message: message.toString() }); return true; };
            iframe.src = url;
        });
    };
    ...
    const tabRenderers = {
        renderAuditoriaTab: async function() {
