document.addEventListener("DOMContentLoaded",()=>{if(window.self!==window.top){return}const e=document.createElement("script");e.src="js/vendor/axe.min.js",document.head.appendChild(e);const t=`
        ...
        <div id="dev-panel" class="hidden fixed bottom-0 left-0 w-full h-2/3 bg-gray-900 text-white shadow-2xl z-[99] flex flex-col font-mono text-sm">
            ...
            <div class="flex-grow overflow-hidden flex">
                ...
                <div id="tab-network" class="dev-tab-content hidden flex-grow flex p-2 overflow-hidden">
                     <div id="network-log" class="w-1/2 overflow-y-auto border-r border-gray-700 pr-2"></div>
                     <div id="network-preview" class="w-1/2 overflow-y-auto pl-2 text-xs whitespace-pre-wrap break-all">Clique em uma requisição para inspecionar.</div>
                </div>
                <div id="tab-recursos" class="dev-tab-content hidden flex-grow flex flex-col p-2 gap-2">
                    ...
                    <div class="flex-grow overflow-auto bg-black bg-opacity-20 rounded">
                        <table class="w-full text-left text-xs">
                            <thead class="sticky top-0 bg-gray-800"><tr><th class="p-2">Nome</th><th class="p-2">Tipo</th><th class="p-2">Tamanho</th><th class="p-2">Tempo (ms)</th></tr></thead>
            ...
            <div class="flex-shrink-0 bg-gray-800 p-1 border-t border-gray-700 flex items-center gap-2">
                <div id="dev-panel-status-indicator" class="relative flex h-3 w-3"><span class="ping-pulse ping-green"></span><span class="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span></div>
                <span id="dev-panel-status-text">Status: OK</span>
            </div>
        </div>
    `;document.body.insertAdjacentHTML("beforeend",t);const n=document.getElementById("dev-tools-trigger"),o=document.getElementById("dev-panel"),l=document.getElementById("dev-panel-close"),d=document.querySelectorAll(".dev-tab"),c=document.querySelectorAll(".dev-tab-content"),r=document.getElementById("console-output"),s=document.getElementById("console-input"),i=document.getElementById("dev-panel-status-indicator"),a=document.getElementById("dev-panel-status-text"),u=[];let m=-1,p=-1;const y={};function h(e,t){const n=i.querySelector(".ping-pulse"),o=i.querySelector(".relative.inline-flex");n.className="ping-pulse",o.className="relative inline-flex rounded-full h-3 w-3";const l={ok:{pulse:"ping-green",dot:"bg-green-500",text:"Status: OK"},error:{pulse:"ping-red",dot:"bg-red-500",text:`Status: Erro (${t})`}},d=l[e]||l.ok;n.classList.add(d.pulse),o.classList.add(d.dot),a.textContent=d.text}["log","warn","info","error"].forEach(e=>{y[e]=console[e],console[e]=(...t)=>{y[e](...t);const n={log:"📝",info:"ℹ️",warn:"⚠️",error:"❌"},o={log:"text-gray-300",info:"text-blue-400",warn:"text-yellow-400",error:"text-red-400"},l=document.createElement("div");l.className=`flex items-start gap-2 border-b border-gray-800 py-1 ${o[e]}`;const d=new Date().toLocaleTimeString();let c=`<span class="flex-shrink-0">${n[e]}</span> <span class="flex-shrink-0 text-gray-500">${d}</span> <div class="break-all">`;c+=[...t].map(e=>"object"==typeof e?JSON.stringify(e,null,2):e).join(" "),"</div>",l.innerHTML=c,r.appendChild(l),r.scrollTop=r.scrollHeight,u.push({type:e,timestamp:d,args:[...t]})}});const f=()=>{const e=document.getElementById("dom-tree");e.innerHTML="",v(document.documentElement,e)},b=()=>{const e=document.getElementById("resources-list"),t=document.getElementById("resources-summary"),n=performance.getEntriesByType("resource");let o=0;e.innerHTML="",n.forEach(t=>{const n=t.name.split("/").pop().split("?")[0],l=(t.transferSize/1024).toFixed(2);o+=t.transferSize;const d=`
    ...
    `};function v(e,t,n=0){if(["SCRIPT","STYLE","LINK","META"].includes(e.tagName))return;const o=document.createElement("details");o.style.marginLeft=`${15*n}px`,o.open=n<2;const l=document.createElement("summary");l.className="cursor-pointer hover:bg-gray-700 p-1 rounded",l.textContent=`<${e.tagName.toLowerCase()}>`,l.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation(),o.open=!o.open,w(e)}),o.appendChild(l),[...e.children].forEach(e=>v(e,o,n+1)),t.appendChild(o)}function w(e){const t=document.getElementById("style-inspector");t.innerHTML="";const n=document.createElement("h3");n.className="font-bold border-b border-gray-700 mb-2 pb-1",n.innerHTML=`Styles for <code>&lt;${e.tagName.toLowerCase()}&gt;</code>`,t.appendChild(n);const o=document.createElement("div");o.className="p-2 bg-gray-800 rounded text-xs";const l=window.getComputedStyle(e);for(const d of l)o.innerHTML+=`<div><span class="text-purple-400">${d}</span>: <span class="text-green-400">${l.getPropertyValue(d)}</span>;</div>`;t.appendChild(o)}const E=document.getElementById("network-log"),k=document.getElementById("network-preview"),S=window.fetch;function L(e){e.preventDefault(),e.stopPropagation();const t=e.target;document.body.style.cursor="default",t.style.outline="",w(t),document.querySelectorAll("*:not(#dev-panel):not(#dev-panel *)").forEach(e=>{e.removeEventListener("mouseover",x),e.removeEventListener("mouseout",A),e.removeEventListener("click",L)})}function x(e){e.target.style.outline="2px solid #38bdf8"}function A(e){e.target.style.outline=""}window.fetch=function(...e){const t=performance.now(),n=e[0]instanceof Request?e[0].url:e[0],o=document.createElement("div");return o.className="flex justify-between items-center p-1 border-b border-gray-800 cursor-pointer hover:bg-gray-700/50",o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span> <span class="loader"></span>`,o.addEventListener("click",async()=>{k.textContent="Carregando pré-visualização...";try{k.textContent=await(await S(n)).text()}catch(e){k.textContent=`Falha ao carregar o recurso: ${e.message}`}}),E.prepend(o),S.apply(this,e).then(e=>{const l=(performance.now()-t).toFixed(2),d=e.ok?"text-green-500":"text-red-500";return o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span><span class="flex items-center gap-2"><span class="${d}">${e.status}</span><span class="text-gray-500">${l}ms</span></span>`,e}).catch(e=>{const n=(performance.now()-t).toFixed(2);throw o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span><span class="flex items-center gap-2"><span class="text-red-500">Erro</span><span class="text-gray-500">${n}ms</span></span>`,e})};const T=document.getElementById("run-accessibility-test"),C=document.getElementById("accessibility-results");function P(e,t,n){const o={minor:"bg-yellow-800 text-yellow-300",moderate:"bg-orange-800 text-orange-300",serious:"bg-red-800 text-red-300",critical:"bg-red-700 text-red-200 font-bold"};let l="";return e.nodes&&e.nodes.length>0&&(l="<ul>",e.nodes.forEach(e=>{l+=`<li class="mt-1"><code class="text-xs bg-gray-700 p-1 rounded">${e.target.join(", ")}</code></li>`}),l+="</ul>"),`
        ...
        `}function I(){return new Promise((e,t)=>{let n=0;const o=setInterval(()=>{if("undefined"!=typeof axe)return clearInterval(o),e();(n+=1)>=25&&(clearInterval(o),t(new Error("Erro: A biblioteca de acessibilidade (axe-core) não carregou a tempo.")))},200)})}const M=document.getElementById("test-results");function O(e,t,n){const o={info:"ℹ️",success:"✅",warn:"⚠️",error:"❌"},l={info:"text-blue-400",success:"text-green-400",warn:"text-yellow-400",error:"text-red-400"},d=document.createElement("div");d.className=`flex items-start gap-2 border-b border-gray-800 py-1 ${l[t]}`,d.innerHTML=`<span>${o[t]}</span> <span class="font-bold w-48 flex-shrink-0">${e}</span> <span>${n}</span>`,M.appendChild(d)}n.addEventListener("click",()=>{o.classList.toggle("hidden")}),l.addEventListener("click",()=>{o.classList.add("hidden")}),d.forEach(e=>{e.addEventListener("click",()=>{d.forEach(e=>e.classList.remove("active-tab")),e.classList.add("active-tab");const t=`tab-${e.dataset.tab}`;c.forEach(n=>{n.classList.add("hidden"),n.id===t&&(n.classList.remove("hidden"),"elements"===e.dataset.tab&&f(),"recursos"===e.dataset.tab&&b(),"info"===e.dataset.tab&&g())})})}),document.getElementById("clear-console").addEventListener("click",()=>{r.innerHTML="",u.length=0,h("ok")}),document.getElementById("export-console").addEventListener("click",()=>{const e=new Blob([JSON.stringify(u,null,2)],{type:"application/json"}),t=URL.createObjectURL(e),n=document.createElement("a");n.href=t,n.download="console-log.json",n.click(),URL.revokeObjectURL(t)}),document.getElementById("element-picker-btn").addEventListener("click",()=>{setTimeout(()=>{document.body.style.cursor="crosshair",document.querySelectorAll("*:not(#dev-panel):not(#dev-panel *)").forEach(e=>{e.addEventListener("mouseover",x),e.addEventListener("mouseout",A),e.addEventListener("click",L)})},0)}),document.getElementById("refresh-resources").addEventListener("click",b),T.addEventListener("click",async()=>{C.innerHTML='<p class="text-yellow-400">Analisando...</p>';try{await I();const e=await axe.run();(function(e){C.innerHTML="";const{violations:t,passes:n,incomplete:o}=e;let l=`<div class="mb-4 p-2 rounded bg-gray-800 flex justify-around"><span class="font-bold text-red-500">Violações: ${t.length}</span><span class="font-bold text-yellow-500">Requer Revisão: ${o.length}</span><span class="font-bold text-green-500">Aprovados: ${n.length}</span></div>`;C.insertAdjacentHTML("beforeend",l),t.length>0&&(C.insertAdjacentHTML("beforeend",'<h3 class="text-lg font-bold text-red-400 mb-2">Violações Críticas</h3>'),t.forEach(e=>{const t=P(e,"error","red");C.insertAdjacentHTML("beforeend",t)})),o.length>0&&(C.insertAdjacentHTML("beforeend",'<h3 class="text-lg font-bold text-yellow-400 mt-4 mb-2">Itens que Requerem Revisão Manual</h3>'),o.forEach(e=>{const t=P(e,"warning","yellow");C.insertAdjacentHTML("beforeend",t)})),n.length>0&&0===t.length&&0===o.length&&C.insertAdjacentHTML("beforeend",'<p class="text-green-400 font-bold text-center mt-4">Parabéns! Nenhum problema de acessibilidade encontrado automaticamente.</p>')})(e)}catch(e){console.error("Erro no teste de acessibilidade:",e),C.innerHTML=`<p class="text-red-500">${e.message}</p>`}}),document.getElementById("run-tests").addEventListener("click",async()=>{M.innerHTML="";const e=["css/style.css","js/script.js","js/dev-panel.js","index.html","algoritmos.html","estruturas-de-dados.html","search.html","status.html"];for(const t of e){let e,n;try{const o=await fetch(t,{method:"HEAD",cache:"no-store"});o.ok?(e="success",n="Arquivo encontrado e acessível."):(e="error",n=`Falha ao carregar o arquivo (Status: ${o.status}).`)}catch(o){e="error",n="Erro de rede."}O(t,e,n)}}),window.onerror=(e,t,n,o,l)=>{console.error(`Erro global: ${e} em ${t}:${n}`),h("error","Erro Global")},s.addEventListener("keydown",e=>{const t=s.value.trim();if("Enter"===e.key&&t){p.push(t),m=p.length;try{new Function(`return ${t}`)()}catch(e){return console.error(e.message),void h("error","Console")}s.value=""}else"ArrowUp"===e.key?m>0&&(m--,s.value=p[m]):"ArrowDown"===e.key&&(m<p.length-1?(m++,s.value=p[m]):(s.value="",m=p.length))})});
