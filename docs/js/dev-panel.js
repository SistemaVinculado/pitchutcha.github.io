document.addEventListener("DOMContentLoaded",()=>{if(window.self!==window.top){return}const e=document.createElement("script");e.src="js/vendor/axe.min.js",document.head.appendChild(e);const t=`
        ...
        <div id="dev-panel" class="hidden fixed bottom-0 left-0 w-full h-2/3 bg-gray-900 text-white shadow-2xl z-[99] flex flex-col font-mono text-sm">
            ...
            <div class="flex-grow overflow-hidden flex">
                ...
                <div id="tab-elements" class="dev-tab-content hidden flex-grow flex overflow-hidden">
                    <div class="w-1/2 overflow-y-auto p-2 border-r border-gray-700" id="dom-tree"></div>
    ...
    `};function v(e,t,n=0){if(["SCRIPT","STYLE","LINK","META"].includes(e.tagName))return;const o=document.createElement("details");o.style.marginLeft=`${15*n}px`,o.open=n<2;const l=document.createElement("summary");l.className="cursor-pointer hover:bg-gray-700 p-1 rounded",l.textContent=`<${e.tagName.toLowerCase()}>`,l.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation(),o.open=!o.open,w(e)}),o.appendChild(l),[...e.children].forEach(e=>v(e,o,n+1)),t.appendChild(o)}function w(e){const t=document.getElementById("style-inspector");t.innerHTML="";const n=document.createElement("h3");n.className="font-bold border-b border-gray-700 mb-2 pb-1",n.innerHTML=`Styles for <code>&lt;${e.tagName.toLowerCase()}&gt;</code>`,t.appendChild(n);const o=document.createElement("div");o.className="p-2 bg-gray-800 rounded text-xs";const l=window.getComputedStyle(e);for(const d of l)o.innerHTML+=`<div><span class="text-purple-400">${d}</span>: <span class="text-green-400">${l.getPropertyValue(d)}</span>;</div>`;t.appendChild(o)}const E=document.getElementById("network-log"),k=document.getElementById("network-preview"),S=window.fetch;window.fetch=function(...e){const t=performance.now(),n=e[0]instanceof Request?e[0].url:e[0],o=document.createElement("div");return o.className="flex justify-between items-center p-1 border-b border-gray-800 cursor-pointer hover:bg-gray-700/50",o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span> <span class="loader"></span>`,o.addEventListener("click",async()=>{k.textContent="Carregando pré-visualização...",k.textContent=await(await S(n)).text()}),E.prepend(o),S.apply(this,e).then(e=>{const l=(performance.now()-t).toFixed(2),d=e.ok?"text-green-500":"text-red-500";return o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span><span class="flex items-center gap-2"><span class="${d}">${e.status}</span><span class="text-gray-500">${l}ms</span></span>`,e}).catch(e=>{const n=(performance.now()-t).toFixed(2);throw o.innerHTML=`<span>${n.substring(n.lastIndexOf("/")+1).substring(0,50)}</span><span class="flex items-center gap-2"><span class="text-red-500">Erro</span><span class="text-gray-500">${n}ms</span></span>`,e})};const L=document.getElementById("run-accessibility-test"),x=document.getElementById("accessibility-results");function A(e,t,n){const o={minor:"bg-yellow-800 text-yellow-300",moderate:"bg-orange-800 text-orange-300",serious:"bg-red-800 text-red-300",critical:"bg-red-700 text-red-200 font-bold"};let l="";return e.nodes&&e.nodes.length>0&&(l="<ul>",e.nodes.forEach(e=>{l+=`<li class="mt-1"><code class="text-xs bg-gray-700 p-1 rounded">${e.target.join(", ")}</code></li>`}),l+="</ul>"),`
        ...
        `}function T(){return new Promise((e,t)=>{let n=0;const o=setInterval(()=>{if("undefined"!=typeof axe)return clearInterval(o),e();(n+=1)>=25&&(clearInterval(o),t(new Error("Erro: A biblioteca de acessibilidade (axe-core) não carregou a tempo.")))},200)})}L.addEventListener("click",async()=>{x.innerHTML='<p class="text-yellow-400">Analisando...</p>';try{await T();const e=await axe.run();(function(e){x.innerHTML="";const{violations:t,passes:n,incomplete:o}=e;let l=`<div class="mb-4 p-2 rounded bg-gray-800 flex justify-around"><span class="font-bold text-red-500">Violações: ${t.length}</span><span class="font-bold text-yellow-500">Requer Revisão: ${o.length}</span><span class="font-bold text-green-500">Aprovados: ${n.length}</span></div>`;x.insertAdjacentHTML("beforeend",l),t.length>0&&(x.insertAdjacentHTML("beforeend",'<h3 class="text-lg font-bold text-red-400 mb-2">Violações Críticas</h3>'),t.forEach(e=>{const t=A(e,"error","red");x.insertAdjacentHTML("beforeend",t)})),o.length>0&&(x.insertAdjacentHTML("beforeend",'<h3 class="text-lg font-bold text-yellow-400 mt-4 mb-2">Itens que Requerem Revisão Manual</h3>'),o.forEach(e=>{const t=A(e,"warning","yellow");x.insertAdjacentHTML("beforeend",t)})),n.length>0&&0===t.length&&0===o.length&&x.insertAdjacentHTML("beforeend",'<p class="text-green-400 font-bold text-center mt-4">Parabéns! Nenhum problema de acessibilidade encontrado automaticamente.</p>')})(e)}catch(e){console.error("Erro no teste de acessibilidade:",e),x.innerHTML=`<p class="text-red-500">${e.message}</p>`}});const C=document.getElementById("test-results");document.getElementById("run-tests").addEventListener("click",async()=>{C.innerHTML="";const e=["css/style.css","js/script.js","js/dev-panel.js","index.html","algoritmos.html","estruturas-de-dados.html","search.html","status.html"];for(const t of e){let e,n;try{const o=await fetch(t,{method:"HEAD",cache:"no-store"});o.ok?(e="success",n="Arquivo encontrado e acessível."):(e="error",n=`Falha ao carregar o arquivo (Status: ${o.status}).`)}catch(o){e="error",n="Erro de rede."}P(t,e,n)}});const P=(e,t,n)=>{const o={info:"ℹ️",success:"✅",warn:"⚠️",error:"❌"},l={info:"text-blue-400",success:"text-green-400",warn:"text-yellow-400",error:"text-red-400"},d=document.createElement("div");d.className=`flex items-start gap-2 border-b border-gray-800 py-1 ${l[t]}`,d.innerHTML=`<span>${o[t]}</span> <span class="font-bold w-48 flex-shrink-0">${e}</span> <span>${n}</span>`,C.appendChild(d)};window.onerror=(e,t,n,o,l)=>{console.error(`Erro global: ${e} em ${t}:${n}`),y("error","Erro Global")}});
